...
export default function AdminPropertiesPage() {
  ...
  const reviewMutation = useMutation({
    mutationFn: async (d: { propertyId: number; status: string; reason: string }) => {
    ...
    onSuccess: () => {
      const label = ALL_STATUSES.find(s => s.value === reviewAction)?.label || reviewAction;
    ...
    onError: (error: any) => {
      messageApi.error(error?.response?.data?.message || "Có lỗi xảy ra khi cập nhật trạng thái");
    },
  });
  ...
  const handleOpenReview = (record: AdminPropertyItem, targetStatus: string) => {
  ...
  const handleSubmitReview = () => {
    ...
    reviewMutation.mutate({ propertyId: reviewPropertyId, status: reviewAction, reason: reviewNote.trim() });
  };
  ...
  const pendingReview = (data?.items || []).filter((i) => i.status === "PENDING_REVIEW").length;

  const columns: ColumnsType<AdminPropertyItem> = [
    {
      ...
      render: (_: unknown, record: AdminPropertyItem) => (
        <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
          <div style={{
            ...
            {record.thumbnailUrl ? (
              <img src={record.thumbnailUrl} alt={record.name}
    ...
    },
    {
      ...
      render: (status: string) => {
        const s = PROPERTY_STATUS_MAP[status] || { color: "default", label: status };
        return (
          <Tag color={s.color} style={{ borderRadius: 6, fontWeight: 500, padding: "2px 10px" }}>
            {s.label}
          </Tag>
        );
      },
          ...
          <Space size={4}>
            <Tooltip title="Xem chi tiết">
              <Button type="text" size="small" icon={<EyeOutlined />}
                onClick={() => handleViewDetail(record)} style={{ color: "#2DD4A8" }} />
            </Tooltip>
    ...
    <div>
      ...
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 24, flexWrap: "wrap", gap: 16 }}>
        ...
        <Space>
          ...
          <Button icon={<ReloadOutlined />} onClick={() => queryClient.invalidateQueries({ queryKey: ["admin-properties"] })}>
            Làm mới
          </Button>
        </Space>
      </div>
      ...
      <Row gutter={[16, 16]} style={{ marginBottom: 24 }}>
        {[
          { title: "Tổng danh sách", value: totalProperties, icon: <HomeOutlined />, iconBg: "#e8f8f3", iconColor: "#2DD4A8", change: null },
          { title: "Chờ duyệt", value: pendingReview, icon: <AlertOutlined />, iconBg: "#fff8e6", iconColor: "#faad14", change: null },
          { title: "Đã đăng công khai", value: (data?.items || []).filter(i => i.status === "PUBLISHED").length, icon: <AppstoreOutlined />, iconBg: "#e6f4ff", iconColor: "#1677ff", change: null },
          { title: "Bị ẩn / Cấm", value: (data?.items || []).filter(i => i.status === "HIDDEN" || i.status === "BANNED").length, icon: <StarOutlined />, iconBg: "#fff1f0", iconColor: "#ff4d4f", change: null },
        ].map((stat, i) => (
      ...
      <Card bordered={false} style={{ borderRadius: 12 }} styles={{ body: { padding: 0 } }}>
        ...
        <div style={{ padding: "16px 20px", display: "flex", justifyContent: "space-between", alignItems: "center", flexWrap: "wrap", gap: 12, borderBottom: "1px solid #f0f0f0" }}>
          <div style={{ display: "flex", gap: 12, alignItems: "center", flex: "1 1 300px", flexWrap: "wrap" }}>
            <Input
              ...
              style={{ flex: 1, minWidth: 200, maxWidth: 320, borderRadius: 8, height: 38 }}
              allowClear
            />
        ...
        <Table
          ...
          pagination={{
            ...
            showTotal: (total, range) => `Đang hiển thị ${range[0]}-${range[1]} trên ${total} kết quả`,
      ...
      <Drawer
        ...
        onClose={() => { setDetailDrawerOpen(false); setSelectedProperty(null); }}
        styles={{ body: { padding: 0 } }}
        extra={
          selectedProperty && selectedProperty.status !== "PUBLISHED" && (
            <Dropdown
              menu={{
                items: ALL_STATUSES
                  .filter(s => s.value !== selectedProperty.status)
                  .map(s => ({
                    key: s.value,
                    label: <span style={{ color: s.color }}>{s.label}</span>,
                    onClick: () => {
      ...
      >
        {selectedProperty ? (
          <>
                ...
                <>
                  ...
                  <div style={{
                    ...
                    <div style={{ fontSize: 13, color: "rgba(255,255,255,0.8)", marginTop: 4 }}>
                      ...
                      {[selectedProperty.district, selectedProperty.province].filter(Boolean).join(", ") || "--"}
                    </div>
                  </div>
                </>
              ) : (
            ...
            <div style={{ padding: "20px 24px" }}>
              ...
              <div style={{ display: "flex", gap: 8, marginBottom: 20 }}>
                <Tag
                  color={PROPERTY_STATUS_MAP[selectedProperty.status]?.color || "default"}
                  style={{ borderRadius: 6, fontWeight: 500, padding: "4px 14px", fontSize: 13 }}
                >
                  {PROPERTY_STATUS_MAP[selectedProperty.status]?.label || selectedProperty.status}
                </Tag>
                {selectedProperty.categoryName && (
                  <Tag style={{ borderRadius: 6, padding: "4px 12px", fontSize: 13 }}>
                    {selectedProperty.categoryName}
                  </Tag>
                )}
              </div>

              {/* Info Descriptions — only real API fields */}
              <Descriptions
                bordered
                size="small"
                column={1}
                labelStyle={{ fontWeight: 600, width: 140, background: "#fafafa", fontSize: 13 }}
                contentStyle={{ fontSize: 13 }}
              >
                <Descriptions.Item
                  label={<><UserOutlined style={{ marginRight: 6 }} />Chủ nhà</>}
                >
                  <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                    <Avatar src={selectedProperty.hostAvatarUrl} icon={<UserOutlined />} size={28} />
                    <div>
                      <div style={{ fontWeight: 500 }}>{selectedProperty.hostName || "--"}</div>
                      <div style={{ fontSize: 12, color: "#94a3b8" }}>{selectedProperty.hostEmail || "--"}</div>
                    </div>
                  </div>
                </Descriptions.Item>
                <Descriptions.Item
                  label={<><EnvironmentOutlined style={{ marginRight: 6 }} />Tỉnh / TP</>}
                >
                ...
                <Descriptions.Item
                  label={<><CalendarOutlined style={{ marginRight: 6 }} />Ngày tạo</>}
                >
                  {selectedProperty.createdAt
                    ? dayjs(selectedProperty.createdAt).format("DD/MM/YYYY HH:mm")
                    : "--"}
                </Descriptions.Item>
              </Descriptions>
            </div>
          ...
          <Empty description="Không có dữ liệu" style={{ padding: "80px 0" }} />
        )}
      </Drawer>
      ...
      <Modal
        title={
          <Space>
            <span>{ALL_STATUSES.find(s => s.value === reviewAction)?.label || reviewAction}</span>
          </Space>
        }
        ...
        okText="Xác nhận"
        okButtonProps={{
          danger: reviewAction === "REJECTED" || reviewAction === "BANNED",
          style: (reviewAction === "PUBLISHED")
      ...
      >
        <div>
          <p style={{ marginBottom: 12, color: "#475569" }}>
            ...
            <strong style={{ color: ALL_STATUSES.find(s => s.value === reviewAction)?.color }}>
